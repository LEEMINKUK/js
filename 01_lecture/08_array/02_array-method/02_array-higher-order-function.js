...
number.sort(compare); // 오름차순 정렬
console.log(number);

number.sort((a, b) => b - a);
console.log(number); // 내림차순 정렬
...
// Array.prototype.some : 배열 내 일부 요소가 콜백 함수의 테스트를 통과하는지 확인하여 그 결과를 boolean 으로 반환
let result = [1,5,3,2,4].some(item => item > 10);
console.log(result);
let result2 = [1,5,3,2,4].some(item => item > 3);
console.log(result2);

// Array.prototype.every : 배열 내 모든 요소가 통과해야 그 결과를 true 로 반환
...
let result4 = student.find(item => item.name === '유관순');
console.log(result4);
let result5 = student.find(item => item.name === '신사임당');
console.log(result5);
let result6 = student.findIndex(item => item.name === '유관순');
console.log(result6);
let result7 = student.findIndex(item => item.name === '신사임당');
console.log(result7);