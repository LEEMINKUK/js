...
// length 프로퍼티 값은 배열에 요소를 추가하거나 삭제하면 자동 갱신 된다.
arr.push(6);   // 인자로 전달한 요소 추가 하는 메서드
console.log(arr);
arr.pop();     // 마지막 요소 제거 하는 메서드
console.log(arr);
...
// 현재 length 프로퍼티보다 큰 숫자 값을 할당하면 length 프로퍼티의 값도 변경되지만 배열의 길이가 늘어난다.
arr.length = 10;
console.log(arr);
